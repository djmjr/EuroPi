...
class SquareOscillator:
    ...
    def get_pitch(self, hertz):
        # Check to make sure hertz isn't < 0
        #if hertz < 0:
        #    hertz = 0
        #else: hertz
...
class PolySquareMods(EuroPiScript):
    def __init__(self):
        ...
        # list above.
        self.modes = [
            PolyphonyMode("Unison", (0, 0, 0, 1, 1, -1)),
            PolyphonyMode("5th", (0, 0, 7/12, 1, 19/12, -5/12)),
            PolyphonyMode("Octave", (0, 0, 1, 1, 2, -1)),
            PolyphonyMode("Power chord", (0, 7/12, 1, 1, 19/12, -5/12)),
            PolyphonyMode("Stacked 5ths", (0, 7/12, 14/12, 1, 19/12, -5/12)),
            PolyphonyMode("Minor triad", (0, 7/12, 15/12, 1, 19/12, -5/12)),
            PolyphonyMode("Major triad", (0, 7/12, 16/12, 1, 19/12, -5/12)),
            PolyphonyMode("Diminished", (0, 6/12, 15/12, 1, 18/12, -6/12)),
            PolyphonyMode("Augmented", (0, 8/12, 16/12, 1, 20/12, -4/12)),
            PolyphonyMode("Major 6th", (0, 4/12, 9/12, 1, 16/12, -8/12)),
            PolyphonyMode("Major 7th", (0, 4/12, 11/12, 1, 16/12, -8/12)),
            PolyphonyMode("Minor 7th", (0, 3/12, 10/12, 1, 15/12, -9/12))
        ]
        self.current_mode = None
        self.ui_update_requested = True
        self.detune_amount = 0
        self.coarse_tune = .5
        self.fine_tune = .5
    ...
    def update_ui(self):
        if self.tuning_mode:
            text = ("Tuning Mode\ncoarse: " + str("{:.3f}".format(self.coarse_tune - .5)) +
                "\nfine: " + str("{:.2f}".format(self.fine_tune - .5)))
        else:
            text = ("Super Square\n" + self.modes[self.current_mode].name + "\nDetune %:" + str("{:.0f}".format(self.detune_amount*1200)))
    ...
    def update_settings(self):
        ...
        if self.tuning_mode:
            ...
            new_detune = k1.read_position(100) / 1200
